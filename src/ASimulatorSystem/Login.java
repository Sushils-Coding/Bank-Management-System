        ...
        try {
            if (ae.getSource() == b1) {
                ...
                if (cardno.isEmpty() || pin.isEmpty()) {
                    JOptionPane.showMessageDialog(this, "Card Number or PIN cannot be empty");
                    return;
                }

                // Using PreparedStatement
                String query = "SELECT * FROM login WHERE cardNumber = ? AND pin = ?";
                PreparedStatement stmt = c1.c.prepareStatement(query); // Use Connection from Conn
                ...
                if (rs.next()) {
                    setVisible(false);
                    new Transactions(pin).setVisible(true); // Navigate to Transactions
                } else {
